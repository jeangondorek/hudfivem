$(document).ready(function(){
	window.addEventListener("message",function(event){
		if (event.data.hud == true){
			...
			$("#logoserver").css("display","block");
		}

		if (event.data.hud == false){
			$("#hudDisplay").css("display","none");
			$("#logoserver").css("display","none");
		}
		...
		$(".infosBack").html(event.data.radio +"<b>" +"</b>  "+ event.data.street);
		...
		if (event.data.sede > 100){
			$(".thirstDisplay").css("width","0");
		} else {
			$(".thirstDisplay").css("width",100-(event.data.sede) +"%");
		}

		if (event.data.fome > 100){
			$(".hungerDisplay").css("width","0");
		} else {
